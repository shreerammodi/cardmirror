...
| **Reformat Every Cite** | Search Everything → "reformat all cites" (unbound; assign a key in Settings → Keyboard) | Runs **Format Cite** over every cite in the open document, several at a time. The first cite goes on its own; after that the pass speeds up for as long as your AI provider keeps up, and eases off the moment it asks you to slow down — so it goes as fast as your key allows without turning a long run into failed cites. If the provider does ask for a pause, the pass waits it out (the pill says so) instead of giving up on that cite. Because it is still one model request per cite, a confirmation first tells you how many requests it will make, and the pass never makes more than that — cites pasted in mid-run are left for a second pass. Every cite is locked while the pass runs; the cites being worked on right now are outlined, and the pill counts rewrites as they land. Each cite is its own undo step, **Esc** stops the pass after the cites already in flight, and a summary reports how many were reformatted, failed, or were left unstyled. Three failures in a row (a rejected key, an exhausted quota) end the run instead of repeating the same failure on every remaining cite. One document can only have one pass running at a time, but in three-pane mode each pane can run its own, and **Esc** then stops the pass in the pane you are working in. |
